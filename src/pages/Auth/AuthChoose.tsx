...
export const AuthChoose: FC<AuthChooseProps> = ({
  title,
  desc,
  image,
  sign,
  ...
  <div className="col-md-12  z-2 mt-6 transition-transform duration-1000 transform hover:scale-105 ">
    ...
    <div
      onClick={sign ? handleButtonClick : undefined}
      className="bg-[#fff] w-full flex fle-row align-center justify-between p-4 transition-colors hover:bg-blue-100 cursor-pointer h-[50px] md:h-[70px] lg:h-[80px]  rounded-xl shadow-lg "
    >
      <div className="cursor-pointer text-left flex flex-row items-center gap-4 align-center">
        <img
          src={image}
          alt="Fin"
          className="sm:h-[20px] md:h-[55px] lg:h-[65px]"
        />
        <div>
          <p className="text-[#0757bc] mb-[2px] lg:mb-[10px] text-sm lg:text-xl  md:text-lg font-semibold  ">
            {title}
          </p>
          <p className="text-[#7d7d7d] mt-[10px] text-base hidden lg:block md:block">
            {desc}
          </p>
        </div>
      </div>
